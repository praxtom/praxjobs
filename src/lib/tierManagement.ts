...
export class TierManagementService {
    ...
    const initialProfile: UserSubscription = {
      ...
      featureUsage: {
        ...
        linkedinOptimization: {
          usageCount: 0,
          maxAllowedUsage: tierConfig.featureUsageLimits.linkedinOptimization,
          lastResetTimestamp: Timestamp.now()
        }
      },
    ...
    return {
      ...
      featureUsage: {
        resumeGeneration: {
          ...
          limit:
            SUBSCRIPTION_TIERS[tierProfile.currentTier].featureUsageLimits
              .resumeGeneration
        },
        coverLetterGeneration: {
  ...
  static async trackJobTracker (userId: string): Promise<boolean> {
    try {
      ...
      if (jobTrackersUsage.usageCount >= jobTrackersUsage.maxAllowedUsage) {
        throw new Error(
          'You have reached the maximum number of job trackers. Delete an existing tracker to add a new one.'
        )
      }
  ...
  static async trackJobApplication (userId: string): Promise<boolean> {
    try {
      ...
      if (
        jobApplicationsUsage.usageCount >= jobApplicationsUsage.maxAllowedUsage
      ) {
        throw new Error(
          'You have reached the maximum number of job applications. Delete an existing application to add a new one.'
        )
      }
        ...
        userSubscriptionRef,
        {
          ...
          featureUsage: {
            ...
            coverLetterGeneration: {
              usageCount: 0,
              maxAllowedUsage:
                tierConfig.featureUsageLimits.coverLetterGeneration,
              lastResetTimestamp: Timestamp.now()
            },
            jobAnalysis: {
              usageCount: 0,
            ...
            linkedinOptimization: {
              usageCount: 0,
              maxAllowedUsage:
                tierConfig.featureUsageLimits.linkedinOptimization,
              lastResetTimestamp: Timestamp.now()
            }
          },
          subscriptionStartDate: Timestamp.now(),
      ...
      if (isAfter(now, nextResetDate)) {
        ...
        const resetFeatureUsage: UserSubscription['featureUsage'] = {
          resumeGeneration: {
            usageCount: 0,
            maxAllowedUsage:
              SUBSCRIPTION_TIERS[currentTier].featureUsageLimits
                .resumeGeneration,
            lastResetTimestamp: Timestamp.now()
          },
          coverLetterGeneration: {
          ...
          jobAnalysis: {
            usageCount: 0,
            maxAllowedUsage:
              SUBSCRIPTION_TIERS[currentTier].featureUsageLimits.jobAnalysis,
            lastResetTimestamp: Timestamp.now()
          },
          jobTrackers: {
            usageCount: 0,
            maxAllowedUsage:
              SUBSCRIPTION_TIERS[currentTier].featureUsageLimits.jobTrackers,
            lastResetTimestamp: Timestamp.now()
          },
          jobApplications: {
            usageCount: 0,
          ...
          interviewPrep: {
            usageCount: 0,
            maxAllowedUsage:
              SUBSCRIPTION_TIERS[currentTier].featureUsageLimits.interviewPrep,
            lastResetTimestamp: Timestamp.now()
          },
          linkedinOptimization: {
            usageCount: 0,
    ...
    try {
      ...
      const resetFeatureUsage: UserSubscription['featureUsage'] = {
        resumeGeneration: {
          usageCount: 0,
          maxAllowedUsage:
            SUBSCRIPTION_TIERS[currentTier].featureUsageLimits.resumeGeneration,
          lastResetTimestamp: Timestamp.now()
        },
        coverLetterGeneration: {
          usageCount: 0,
        ...
        jobApplications: {
          usageCount: 0,
          maxAllowedUsage:
            SUBSCRIPTION_TIERS[currentTier].featureUsageLimits.jobApplications,
          lastResetTimestamp: Timestamp.now()
        },
        interviewPrep: {
          usageCount: 0,
  ...
  static async initializeUserSubscriptionIfNeeded (
    userId: string
  ): Promise<UserSubscription> {
    try {
      const db = this.getFirestoreInstance()
  ...
  static async trackResumeGeneration (userId: string): Promise<boolean> {
    try {
      ...
      if (
        resumeGenerationUsage.usageCount >=
        resumeGenerationUsage.maxAllowedUsage
      ) {
        throw new Error(
          'You have reached the maximum number of resume generations. Upgrade to continue.'
        )
      }
  ...
  static async trackCoverLetterGeneration (userId: string): Promise<boolean> {
    try {
      const tierProfile = await this.checkAndResetFeatureUsageIfNeeded(userId)
      const coverLetterGenerationUsage =
        tierProfile.featureUsage.coverLetterGeneration

      // Check if user has reached the maximum number of cover letter generations
      if (
        coverLetterGenerationUsage.usageCount >=
        coverLetterGenerationUsage.maxAllowedUsage
      ) {
        throw new Error(
          'You have reached the maximum number of cover letter generations. Upgrade to continue.'
        )
      }
  ...
  static async trackJobAnalysisRequests (userId: string): Promise<boolean> {
    try {
      ...
      if (jobAnalysisUsage.usageCount >= jobAnalysisUsage.maxAllowedUsage) {
        throw new Error(
          'You have reached the maximum number of job analysis requests. Upgrade to continue.'
        )
      }
  ...
  static async trackJobTrackers (userId: string): Promise<boolean> {
    try {
      ...
      if (jobTrackersUsage.usageCount >= jobTrackersUsage.maxAllowedUsage) {
        throw new Error(
          'You have reached the maximum number of job trackers. Upgrade to continue.'
        )
      }
  ...
  static async trackJobApplications (userId: string): Promise<boolean> {
    try {
      ...
      if (
        ...
        throw new Error(
          'You have reached the maximum number of job applications. Upgrade to continue.'
        )
      }
  ...
  static async trackInterviewPrep (userId: string): Promise<boolean> {
    try {
      ...
      if (interviewPrepUsage.usageCount >= interviewPrepUsage.maxAllowedUsage) {
        throw new Error(
          'You have reached the maximum number of interview prep sessions. Upgrade to continue.'
        )
      }
