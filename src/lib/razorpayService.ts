 ...
 ${tierConfig.name} Membership`,
  ...
  private async onSubscriptionActivated (
    ...
    console.log('\ud83d\udd0e Extracted User Details:', {
      ...
      noteKeys: payload.payload.subscription.notes
        ? Object.keys(payload.payload.subscription.notes)
        : 'No notes'
    })

    if (!userId || !tier) {
      console.error(
        '\u274c Subscription Activation Failed: Missing Critical Data',
        {
          userId,
          tier,
  ...
  async createPaymentLink (
    ...
    try {
      ...
      const baseUrl =
        import.meta.env.PUBLIC_BASE_URL || 'https://yourresume.com'
