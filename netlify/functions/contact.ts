...
const handler: Handler = async (event) => {
    ...
    const headers = {
        'Access-Control-Allow-Origin': '*', // Restrict this in production if possible
    ...
    try {
        ...
        if (!name || !email || !subject || !message) {
            ...
            return {
                statusCode: 400, // Bad Request
                headers,
                body: JSON.stringify({ error: 'Missing required fields: name, email, subject, and message' }),
            };
        }
        ...
        return {
            statusCode: 200, // OK
            headers,
            body: JSON.stringify({ message: 'Contact request submitted successfully. We will get back to you soon!' }),
        };
        ...
        return {
            ...
            body: JSON.stringify({ error: 'An internal server error occurred while sending the message.' }),
        };
    }
};
