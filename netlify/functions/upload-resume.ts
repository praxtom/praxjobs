...
async function cleanResumeText(rawText: string): Promise<string> {
  ...
  if (!rawText) {
    // console.error("ERROR: No text provided to cleanResumeText"); // Removed for prod
    return rawText;
  }

  try {
    // Explicit logging of input text
    // console.error("DEBUG: Attempting to clean text"); // Removed for prod
    // console.error("DEBUG: Raw Text Length:", rawText.length); // Removed for prod
    // console.error("DEBUG: First 500 characters:", rawText.slice(0, 500)); // Removed for prod

    // Explicit check for text transmission
    if (rawText.trim().length === 0) {
      // console.error("ERROR: Empty text provided"); // Removed for prod
      return rawText;
    }

    // Explicit error handling for Groq initialization
    if (!groq) {
      // console.error("ERROR: Groq client not initialized"); // Removed for prod
      return rawText;
    }

    // Explicit logging of AI call parameters
    // console.error("DEBUG: Preparing AI call with model: llama-3.1-8b-instant"); // Removed for prod
...
- Do NOT add or remove any information`,
      ...
      // if (aiCallError instanceof Error) { // Removed verbose details for prod
    ...
    // console.error("DEBUG: AI Response received"); // Removed for prod
    // console.error( // Removed for prod
    ...
    // if (error instanceof Error && "response" in error) { // Removed verbose details for prod
